...
export const taskLoader = async ({ params }: TaskLoaderArgs) => {
  ...
  const annotationBatchs = await retryer(async () => await userAnnotationsProvider.getBatchs(user?.id || '', task?.id || ''));
  ...
  if (!!lastAnnotationBatch) {
    ...
    reviews = await retryer(async () => await userAnnotationsProvider.getReviews(user?.id || '', task?.id || '', lastAnnotationBatch?.id || ''));
